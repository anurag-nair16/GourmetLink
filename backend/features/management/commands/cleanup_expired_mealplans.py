...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Log start time
        logger.info(f"Starting cleanup at {timezone.now()}")
        ...
        for plan in expired_plans:
            logger.info(f"Processing expired plan: {plan.name} (ID: {plan.id}) - User: {plan.user.username}")
...
GourmetLink Team""",
                ...
                logger.info(f"Sent expired notification to {user_email}")
        ...
        # Send notifications for plans expiring tomorrow
...
GourmetLink Team""",
                ...
                logger.info(f"Sent expiring soon notification to {plan.user.email}")
            except Exception as e:
                logger.error(f"Failed to send notification to {plan.user.email}: {str(e)}")
